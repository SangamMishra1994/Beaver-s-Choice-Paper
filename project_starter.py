...
from multi_agent_system import MunderDifflinAgent
...
model = OpenAIModel(model_id="gpt-4o", openai_client=client)

inventory_agent = CodeAgent(
    tools=[],
    model=model,
    description="Handles inventory checking and stock availability",
)

quote_agent = CodeAgent(tools=[], model=model, description="Generates pricing quotes")

ordering_agent = CodeAgent(
    tools=[], model=model, description="Handles supplier ordering"
)
...
class InventoryAgent:
    ...
    def check_inventory(self, item_name, quantity, request_date):
        stock_df = get_stock_level(item_name, request_date)
        stock = stock_df["current_stock"].iloc[0]

        if stock >= quantity:
            return {"status": "available", "stock": stock}

        return {"status": "insufficient", "stock": stock}
...
class QuotingAgent:
    ...
    def generate_quote(self, request_text):

        history = search_quote_history(request_text.split())

        history_context = ""
        for h in history:
            history_context += (
                f"Request: {h['original_request']}\nQuote: {h['total_amount']}\n"
            )

        prompt = f"""
You are a pricing expert for a paper supply company.
...
class OrderingAgent:
    ...
    def order_stock(self, item_name, quantity, request_date):
        ...
        unit_price = item["unit_price"].iloc[0]
        total_price = quantity * unit_price

        delivery_date = get_supplier_delivery_date(request_date, quantity)

        create_transaction(
            item_name, "stock_orders", quantity, total_price, request_date
        )

        return f"Ordered {quantity} units of {item_name}. Expected delivery {delivery_date}."
...
class MunderDifflinAgent:
    ...
    def process_request(self, request_text):
        ...
        inventory_status = self.inventory_agent.check_inventory(
            item_name, quantity, date
        )

        if inventory_status["status"] == "insufficient":

            order_response = self.ordering_agent.order_stock(item_name, quantity, date)

        quote = self.quoting_agent.generate_quote(request_text)

        create_transaction(item_name, "sales", quantity, quantity * 0.1, date)
...
def run_test_scenarios():
    ...
    agent_system = MunderDifflinAgent()
    ...
    for idx, row in quote_requests_sample.iterrows():
        ...
        response = agent_system.process_request(request_with_date)
        ...
        decision = "fulfilled" if response else "unfulfilled"
        ...
        results.append(
            {
                ...
                "fulfilled": decision,
                "cash_balance_changed": decision == "fulfilled",
                "reason": (
                    "completed" if decision == "fulfilled" else "stock unavailable"
                ),
